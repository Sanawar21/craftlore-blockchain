...
class AssetCreationHandler:
    ...
    def create_asset(self, context: Context, payload: Dict):
        ...
        if asset_type == AssetType.RAW_MATERIAL:
            ...
            self.account_validator.validate_supplier_account(context, signer_public_key)
    ...
    def _initialize_asset_specific_fields(self, context: Context, asset, payload: Dict, asset_type: AssetType):
        ...
        if asset_type == AssetType.RAW_MATERIAL:
            ...
            if assignee_id:
                self.account_validator.validate_artisan_or_workshop_account(context, assignee_id)
        ...
        try:
            if asset_type == AssetType.RAW_MATERIAL:
                ...
                if hasattr(asset, 'assigner_id') and asset.assigner_id:
                    assigner_account_type = self.account_validator.get_account_type(context, asset.assigner_id)
                    if assigner_account_type == 'workshop':
                        self.account_updater.update_workshop_production(
                            context, asset.assigner_id, asset.asset_id, None, 'work_order_issued', timestamp
                        )
                        
            elif asset_type == AssetType.PRODUCT_BATCH:
                # Update creator account
                creator_account_type = self.account_validator.get_account_type(context, asset.owner)
    ...
    def _update_account_state_for_product_creation(self, context: Context, product_ids: List[str], 
        ...
        try:
            ...
            creator_account_type = self.account_validator.get_account_type(context, creator_public_key)
            
            if creator_account_type == 'artisan':
                # Update artisan performance metrics
                account = self.account_updater.get_account_by_public_key(context, creator_public_key)
                if account:
                    if 'performance_metrics' not in account:
                        account['performance_metrics'] = {'products_completed': 0}
                    
                    account['performance_metrics']['products_completed'] += len(product_ids)
                    
                    self.account_updater.add_account_history(account, {
                        'action': 'products_created_and_sold',
                        'asset_ids': product_ids,
                        'buyer_public_key': buyer_public_key,
                        'details': f'Created and sold {len(product_ids)} products'
                    }, timestamp)
                    
                    self.account_updater.update_account(context, account)
                    
            elif creator_account_type == 'workshop':
                # Update workshop production records
                account = self.account_updater.get_account_by_public_key(context, creator_public_key)
                if account:
                    if 'products_produced' not in account:
                        account['products_produced'] = []
                    
                    account['products_produced'].extend(product_ids)
                    
                    self.account_updater.add_account_history(account, {
                        'action': 'products_created_and_sold',
                        'asset_ids': product_ids,
                        'buyer_public_key': buyer_public_key,
                        'details': f'Produced and sold {len(product_ids)} products'
                    }, timestamp)
