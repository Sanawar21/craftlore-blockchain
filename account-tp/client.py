...
class CreateAccountClient:
    ...
    def __init__(self, base_url='http://rest-api:8008'):
        ...
        self.seralizer = SerializationHelper()
    ...
    def create_account(self, account_type: str, email: str, **kwargs):
    ...
    def _submit_transaction(self, payload):
        ...
        payload['timestamp'] = self.seralizer.get_current_timestamp()
        payload_bytes = json.dumps(payload).encode('utf-8')
        
        # Generate address for this key
        raw_addresses = [
            self.address_generator.generate_account_address(self.public_key),
            self.address_generator.generate_email_index_address(payload['email']) if 'email' in payload else None,
            self.address_generator.generate_type_index_address(payload['account_type']) if 'account_type' in payload else None,
            self.address_generator.generate_bootstrap_address()
        ]

        addresses = [addr for addr in raw_addresses if addr is not None]
        
        # Create transaction header
        txn_header = TransactionHeader(
            family_name=self.address_generator.FAMILY_NAME,
            family_version='1.0',
            inputs=addresses,
            outputs=addresses,
    ...
    def _wait_for_batch_commit(self, batch_id: str, timeout: int = 30):
